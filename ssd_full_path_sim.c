...
PPA allocate_physical_page_with_gc(void) {
if (g_active_block >= NUM_PHYSICAL_BLOCKS - 2) {
        for (int b = 0; b < NUM_PHYSICAL_BLOCKS; b++) {
            // 跳过当前的活跃块，不能回收正在写的块
            if (b == g_active_block) continue;

            int stale_pages = 0;
            for (int p = 0; p < PAGES_PER_BLOCK; p++) {
                if (!g_nand_flash[b * PAGES_PER_BLOCK + p].is_valid) stale_pages++;
            }

            // 贪心策略：如果脏页较多，启动搬运回收
            if (stale_pages > 48) { 
                printf("\n[GC] 启动受害者 Block %d 搬运流程 (有效页: %d)...\n", b, PAGES_PER_BLOCK - stale_pages);
                
                for (int p = 0; p < PAGES_PER_BLOCK; p++) {
                    PPA old_ppa = b * PAGES_PER_BLOCK + p;

                    // 安全检查 1: 只有标记为 is_valid 的页才去解析 LBA
                    if (g_nand_flash[old_ppa].is_valid) {
                        LBA lba;
                        // 注意：这里我们得从之前定义的 OOB 偏移位置取 LBA
                        // 偏移量 = 用户数据(4096) + 用户CRC(4)
                        int oob_offset = USER_DATA_BYTES + USER_CRC_BYTES;
                        memcpy(&lba, g_nand_flash[old_ppa].raw_cell_data + oob_offset, sizeof(LBA));

                        // 安全检查 2: 防止解析出越界的 LBA 导致段错误
                        if (lba < NUM_LOGICAL_PAGES) {
                            // 安全检查 3: 只有映射表依然指向此 PPA，才说明这是“最新”的有效数据
                            if (g_l2p_table[lba] == old_ppa) {
                                // 执行搬运逻辑...
                                // (此处可以调用一个简化的写入过程，或者暂存到 buffer)
                            }
                        }
                    }
                    // 搬运/处理完后，彻底清空该物理页
                    g_nand_flash[old_ppa].is_valid = 0;
                    memset(g_nand_flash[old_ppa].raw_cell_data, 0, CODEWORD_BYTES);
                }
                
                g_active_block = b; g_page_offset = 0;
                return b * PAGES_PER_BLOCK + g_page_offset++;
            }
        }
    }
...
void ftl_write_full_path(LBA lba, const unsigned char *host_user_data)
{
    ...
    PPA new_ppa = allocate_physical_page_with_gc();
...
void ftl_read_full_path(LBA lba)
{
    ...
    if (iter < 0)
    {
        printf("  -> [Error] LDPC UE (不可纠错错误)，触发 RAIN 重构！lba %d ppa %d\n", lba, target_ppa);
        return;
    }
 ...
 * 模块 4: 并发安全机制
 * ========================================== */
pthread_mutex_t g_ftl_mutex = PTHREAD_MUTEX_INITIALIZER;  // 保护 L2P 和 PPA 分配
pthread_mutex_t g_nand_mutex = PTHREAD_MUTEX_INITIALIZER; // 保护物理存储介质

/* ==========================================
 * 线程 A: Host 持续写入任务
 * ========================================== */
void *host_write_thread(void *arg)
{
    LBA lba_counter = 0;
    unsigned char host_buffer[USER_DATA_BYTES];

    while (1)
    {
        lba_counter = rand() % NUM_LOGICAL_PAGES;
        sprintf((char *)host_buffer, "Data_LBA_%d_TS_%ld", lba_counter, time(NULL));

        pthread_mutex_lock(&g_ftl_mutex);
        pthread_mutex_lock(&g_nand_mutex);

        ftl_write_full_path(lba_counter, host_buffer);

        pthread_mutex_unlock(&g_nand_mutex);
        pthread_mutex_unlock(&g_ftl_mutex);
 ...
 * 线程 B: Host 持续读取与校验任务
 * ========================================== */
void *host_read_thread(void *arg)
{
    LBA lba_to_read;

    while (1)
    {
        lba_to_read = rand() % NUM_LOGICAL_PAGES;

        pthread_mutex_lock(&g_ftl_mutex);
        // 读取不需要写锁，但为了防止读到正在被修改的 PPA 指针，仍需保护 L2P 查表
        ftl_read_full_path(lba_to_read);
        pthread_mutex_unlock(&g_ftl_mutex);
...
int main(void)
{
    ...
    printf("========================================================\n");
    printf(" 启动多线程 SSD 全通路并发仿真引擎 (Thread-Safe)\n");
    printf("========================================================\n");

    pthread_t writer_tid, reader_tid;

    // 创建写入线程
    if (pthread_create(&writer_tid, NULL, host_write_thread, NULL) != 0)
    {
    ...
    // 创建读取线程
    if (pthread_create(&reader_tid, NULL, host_read_thread, NULL) != 0)
    {
    ...
    // 主线程监控运行状态，防止程序直接退出
    printf("[Main] 并发引擎已就绪。正在模拟高压 IO 负载...\n");

    // 运行 10 秒后自动退出，或者你可以改为无限循环
    for (int i = 0; i < 1000; i++)
    {
        sleep(1);
        printf("[Main] System Heartbeat - Sec %d\n", i + 1);
    }

    printf("\n[Main] 仿真演练结束，正在回收资源...\n");

    // 实际固件清理应更优雅，这里直接销毁
    pthread_mutex_destroy(&g_ftl_mutex);
    pthread_mutex_destroy(&g_nand_mutex);
