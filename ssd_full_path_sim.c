 ...
 * 全局数据结构
 * ========================================== */
struct ldpc_rom_entry { unsigned short row; unsigned short col; unsigned short shift; };
struct check_node { int *connected_vns; int degree; };
struct var_node { int *connected_cns; int degree; };
...
void bits_to_bytes(const unsigned char *bits, unsigned char *bytes, int num_bytes) {
    for (int i = 0; i < num_bytes; i++) {
        unsigned char val = 0;
        for (int b = 0; b < 8; b++) val |= (bits[i * 8 + b] << b);
        bytes[i] = val;
    }
}
...
void pack_nand_page(const unsigned char *user_data, const unsigned char *meta_data, unsigned char *ldpc_payload) {
    int offset = 0;
    memcpy(ldpc_payload + offset, user_data, USER_DATA_BYTES); offset += USER_DATA_BYTES;
    unsigned int user_crc = calculate_crc32(user_data, USER_DATA_BYTES);
    memcpy(ldpc_payload + offset, &user_crc, USER_CRC_BYTES); offset += USER_CRC_BYTES;
    
    memcpy(ldpc_payload + offset, meta_data, META_DATA_BYTES); offset += META_DATA_BYTES;
...
int verify_nand_page(const unsigned char *decoded_payload) {
    ...
    if (calc_user_crc != read_user_crc) return -1;
    ...
    if (calc_meta_crc != read_meta_crc) return -2;
    ...
    for (j = 0; j < NUM_DATA_BLOCKS; j++) {
        for (w = 0; w < COLUMN_WEIGHT; w++) {
            ...
            do {
                ...
                if (++attempts > LIFTING_FACTOR * 2) { restart_count++; goto restart_matrix; }
            } while (!is_valid);
    ...
    for (j = 0; j < NUM_DATA_BLOCKS; j++) {
        for (int i = 0; i < NUM_CHK_BLOCKS; i++) {
            if (temp_matrix[i][j] != -1) {
                g_rom_base_matrix[current_entry].row = i; g_rom_base_matrix[current_entry].col = j;
                g_rom_base_matrix[current_entry].shift = temp_matrix[i][j]; current_entry++;
            }
        }
    }
...
int ldpc_engine_init(void) {
    ...
    for (entry_idx = 0; entry_idx < g_rom_matrix_entries; entry_idx++) {
        for (z_offset = 0; z_offset < LIFTING_FACTOR; z_offset++) {
            ...
            g_check_nodes[cn_idx].degree++; g_var_nodes[vn_idx].degree++;
        }
    }
    for (i = 0; i < PARITY_BITS; i++) { g_check_nodes[i].degree++; g_var_nodes[LDPC_PAYLOAD_BITS + i].degree++; }

    for (i = 0; i < PARITY_BITS; i++) { g_check_nodes[i].connected_vns = (int *)malloc(g_check_nodes[i].degree * sizeof(int)); g_check_nodes[i].degree = 0; }
    for (i = 0; i < CODEWORD_BITS; i++) { g_var_nodes[i].connected_cns = (int *)malloc(g_var_nodes[i].degree * sizeof(int)); g_var_nodes[i].degree = 0; }
...
int ldpc_decode_soft(double *channel_llr, unsigned char *out_codeword) {
    ...
    for (i = 0; i < PARITY_BITS; i++) {
        ...
        if (g_check_nodes[i].degree > max_node_degree) max_node_degree = g_check_nodes[i].degree;
    }
    ...
    for (iter = 0; iter < MAX_ITERATIONS; iter++) {
        for (i = 0; i < PARITY_BITS; i++) {
            ...
            for (edge_idx = 0; edge_idx < g_check_nodes[i].degree; edge_idx++) {
                ...
                if (current_mag < min_mag1) { min_mag2 = min_mag1; min_mag1 = current_mag; min_mag1_idx = edge_idx; }
                else if (current_mag < min_mag2) { min_mag2 = current_mag; }
            }
            for (edge_idx = 0; edge_idx < g_check_nodes[i].degree; edge_idx++) {
        ...
        for (j = 0; j < CODEWORD_BITS; j++) {
            ...
            for (edge_idx = 0; edge_idx < g_var_nodes[j].degree; edge_idx++) {
                ...
                for (search_idx = 0; search_idx < g_check_nodes[target_cn].degree; search_idx++) {
                    if (g_check_nodes[target_cn].connected_vns[search_idx] == j) { back_edge_idx = search_idx; break; }
                }
        ...
        for (i = 0; i < PARITY_BITS; i++) {
            ...
            if (current_syndrome != 0) { is_all_zero = 0; break; }
        }

        if (is_all_zero) {
            for (i = 0; i < PARITY_BITS; i++) free(msg_chk_to_var[i]);
            free(msg_chk_to_var); free(posteriori_llr); free(msg_var_to_chk);
            return iter + 1;
        }
    }
    for (i = 0; i < PARITY_BITS; i++) free(msg_chk_to_var[i]);
 ...
 * ========================================== */
void ftl_init(void) {
    for (int i = 0; i < NUM_LOGICAL_PAGES; i++) g_l2p_table[i] = INVALID_PPA;
...
PPA allocate_physical_page(void) {
    if (g_page_offset >= PAGES_PER_BLOCK) { g_active_block++; g_page_offset = 0; }
    if (g_active_block >= NUM_PHYSICAL_BLOCKS) { printf("[Fatal] 闪存已满！\n"); exit(-1); }
...
void ftl_write_full_path(LBA lba, const unsigned char *host_user_data) {
    ...
    if (g_l2p_table[lba] != INVALID_PPA) g_nand_flash[g_l2p_table[lba]].is_valid = 0; // 标记脏块
...
void ftl_read_full_path(LBA lba) {
    ...
    if (target_ppa == INVALID_PPA) { printf("[FW Read]  LBA %04d => [空数据]\n", lba); return; }
    ...
    if (iter < 0) { printf("  -> [Error] LDPC UE (不可纠错错误)，触发 RAIN 重构！\n"); return; }
    ...
    if (crc_res == -1) { printf("  -> [Error] User CRC 校验失败 (发生了静默数据损坏 SDC)！\n"); return; }
    if (crc_res == -2) { printf("  -> [Error] Meta CRC 校验失败！\n"); return; }

    // 5. 校验 FTL 元数据反向映射
    LBA read_lba;
    memcpy(&read_lba, decoded_payload + USER_DATA_BYTES + USER_CRC_BYTES, sizeof(LBA));
    if (read_lba != lba) { printf("  -> [Error] 地址错乱！读出的 OOB 指向 LBA %d！\n", read_lba); return; }
...
int main(void) {
    ...
    /* 清理内存 */
    for (int i = 0; i < PARITY_BITS; i++) free(g_check_nodes[i].connected_vns);
    for (int i = 0; i < CODEWORD_BITS; i++) free(g_var_nodes[i].connected_cns);
    free(g_check_nodes); free(g_var_nodes); free(g_rom_base_matrix);
